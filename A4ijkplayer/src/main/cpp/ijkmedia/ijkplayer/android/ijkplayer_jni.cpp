...
#ifdef __cplusplus
        jobject global_media_data_source = env->NewGlobalRef(media_data_source);
#else
...
static int64_t jni_set_ijkio_androidio(JNIEnv* env, jobject thiz, jobject ijk_io)
{
    ...
    if (ijk_io) {
#ifdef __cplusplus
        jobject global_ijkio_androidio = env->NewGlobalRef(ijk_io);
#else
...
static void
IjkMediaPlayer_prepareAsync(JNIEnv *env, jobject thiz, jobject assetMgr)
{
    MPTRACE("%s\n", __func__);
    int retval = 0;
    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
...
IjkMediaPlayer_setOption(JNIEnv *env, jobject thiz, jint category, jobject name, jobject value)
{
...
#ifdef __cplusplus
    c_name = env->GetStringUTFChars(static_cast<jstring>(name), NULL);
#else
    ...
    if (value) {
#ifdef __cplusplus
        c_value = env->GetStringUTFChars(static_cast<jstring>(value), NULL);
#else
...
LABEL_RETURN:
    if (c_name)
#ifdef __cplusplus
        env->ReleaseStringUTFChars(static_cast<jstring>(name), c_name);
#else
        (*env)->ReleaseStringUTFChars(env, name, c_name);
#endif
    if (c_value)
#ifdef __cplusplus
        env->ReleaseStringUTFChars(static_cast<jstring>(value), c_value);
#else
...
IjkMediaPlayer_getColorFormatName(JNIEnv *env, jclass clazz, jint mediaCodecColorFormat)
{
    ...
    if (!codec_name)
        return NULL ;
#ifdef __cplusplus
    return env->NewStringUTF(codec_name);
#else
...
IjkMediaPlayer_getVideoCodecInfo(JNIEnv *env, jobject thiz)
{
    ...
    if (ret < 0 || !codec_info)
        goto LABEL_RETURN;
#ifdef __cplusplus
    jcodec_info = env->NewStringUTF(codec_info);
#else
...
//    size_t count = ijkmeta_get_children_count_l(meta);
    for (size_t i = 0; i < ijkmeta_get_children_count_l(meta); ++i) {
...
#ifdef __cplusplus
    ijkmp_set_weak_thiz(mp, env->NewGlobalRef(weak_this));
#else
...
static void message_loop_n(JNIEnv *env, IjkMediaPlayer *mp)
{
    ...
    while (1) {
        ...
        case FFP_MSG_GET_IMG_STATE:
            if (msg.obj) {
#ifdef __cplusplus
                jstring file_name = env->NewStringUTF((char *)msg.obj);
#else
                ...
                J4A_DeleteLocalRef__p(env, (jobject *)&file_name);
            }
            else {
...
#ifdef __cplusplus
    c_lib_name = env->GetStringUTFChars(libName, NULL);
#else
...
LABEL_RETURN:
    if (c_lib_name)
#ifdef __cplusplus
        env->ReleaseStringUTFChars(libName, c_lib_name);
#else
...
static JNINativeMethod g_methods[] = {
    ...
    { "_prepareAsync",          "(Landroid/content/res/AssetManager;)V",      (void *) IjkMediaPlayer_prepareAsync },
...
#ifdef __cplusplus
    IJK_FIND_JAVA_CLASS_CPP(env, g_clazz.clazz, JNI_CLASS_IJKPLAYER);
