...
{
    char *tok = NULL;
    tok = strchr((char *)ass, ':'); if (tok) tok += 1; // skip event
        ...
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (is->subtitle_st) {
                while (frame_queue_nb_remaining(&is->subpq) > 0) {
                    ...
                    {
                        if (sp->uploaded) {
                            ffp_notify_msg4(ffp, FFP_MSG_TIMED_TEXT, 0, 0, (void *)"", 1);
                        }
...
reload:
    ...
    if (af->frame->format        != is->audio_src.fmt            ||
        ...
        is->swr_ctx = swr_alloc_set_opts(NULL,
                                         ...
                                         dec_channel_layout,
                                         (enum AVSampleFormat) af->frame->format, af->frame->sample_rate,
                                         0, NULL);
        if (!is->swr_ctx) {
            av_log(NULL, AV_LOG_ERROR,
                   "Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d Hz %s %d channels!\n",
                   af->frame->sample_rate, av_get_sample_fmt_name(
                            (enum AVSampleFormat) af->frame->format), af->frame->channels,
...
void ffp_set_frame_at_time(FFPlayer *ffp, const char *path, int64_t start_time, int64_t end_time, int num, int definition) {
    if (!ffp->get_img_info) {
        ffp->get_img_info = (GetImgInfo *) av_mallocz(sizeof(GetImgInfo));
        if (!ffp->get_img_info) {
...
void *ffp_set_ijkio_inject_opaque(FFPlayer *ffp, void *opaque)
{
    ...
    ijkio_manager_set_callback(ffp->ijkio_manager_ctx, (void *) ijkio_app_func_event);
...
bool loadModel(JNIEnv *env, jobject assetManager) {
    ...
    const char *modeltype = "m";
    const int target_size = 320;
    const float mean_val[3] = {103.53f, 116.28f, 123.675f};
    const float norm_val[3] = {1.f / 57.375f, 1.f / 57.12f, 1.f / 58.395f};
    ncnn::MutexLockGuard g(lock);
    if (!g_nanodet) {
